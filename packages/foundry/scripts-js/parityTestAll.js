#!/usr/bin/env node
/**
 * Full Parity Test - Oracle JS vs On-Chain ChallengeExecutor
 * 
 * This script compares the Oracle's JavaScript simulation results with what 
 * the on-chain ChallengeExecutor would compute, ensuring they produce identical
 * results (tiles discovered, mushrooms found, payout).
 * 
 * Usage: 
 *   node scripts-js/parityTestAll.js [seed]
 *   
 * If no seed provided, generates a random one.
 * 
 * To compare with on-chain results, run the Foundry test:
 *   forge test --match-test test_FullE2E_RealSimulation -vvv
 * 
 * Then compare the output values.
 */
...
// ============================================================================
// Full Simulation (matches Oracle simulateGame)
// ============================================================================

function runSimulation(seed) {
  console.log(`\n${'='.repeat(70)}`);
  console.log(`PARITY TEST - JavaScript Simulation`);
  ...
  console.log(`Terrain: ${JSON.stringify(terrainCounts)}`);
  ...
  console.log(`Spawn: tile(${spawnRow},${spawnCol}) -> world(${spawn.x}, ${spawn.y})`);
...
// Get seed from args or generate random
let seed = process.argv[2];
if (!seed) {
  // Generate random seed
  const randomBytes = crypto.getRandomValues(new Uint8Array(32));
  seed = '0x' + Array.from(randomBytes).map(b => b.toString(16).padStart(2, '0')).join('');
}
...
console.log(`\n${'='.repeat(70)}`);
console.log(`JAVASCRIPT SIMULATION RESULTS`);
console.log(`${'='.repeat(70)}`);
console.log(`  Agent count:      ${result.agentCount}`);
console.log(`  Tiles discovered: ${result.tilesDiscovered}`);
console.log(`  Mushrooms found:  ${result.mushroomsFound}`);
console.log(`  Payout:           ${formatEther(result.payout)} ETH (${result.payout} wei)`);

console.log(`\n${'='.repeat(70)}`);
console.log(`TO VERIFY ON-CHAIN PARITY:`);
console.log(`${'='.repeat(70)}`);
console.log(`\n1. Run the Foundry test with this seed:`);
console.log(`   cd packages/foundry`);
console.log(`   forge test --match-test test_FullE2E_RealSimulation -vvv`);
console.log(`\n2. Compare these values with Solidity output:`);
console.log(`   - Agent count should be: ${result.agentCount}`);
console.log(`   - Tiles discovered should be: ${result.tilesDiscovered}`);
console.log(`   - Mushrooms found should be: ${result.mushroomsFound}`);
console.log(`   - Payout should be: ${result.payout} wei`);

console.log(`\n${'='.repeat(70)}`);
console.log(`COPY-PASTE FOR FOUNDRY TEST:`);
console.log(`${'='.repeat(70)}`);
console.log(`\nbytes32 constant TEST_SEED = ${seed};`);
console.log(`uint256 constant EXPECTED_TILES = ${result.tilesDiscovered};`);
console.log(`uint256 constant EXPECTED_MUSHROOMS = ${result.mushroomsFound};`);
console.log(`uint256 constant EXPECTED_PAYOUT = ${result.payout};`);
console.log(`uint32 constant EXPECTED_AGENTS = ${result.agentCount};`);
